    ...
    public static int surfaceArea(List<List<int>> A)
    {
        ...
        for (int i = 0; i < H; i++)
        {
            for (int j = 0; j < W; j++)
            {
                ...
                int north = (i == 0) ? 0 : A[i - 1][j];
                area += Math.Max(0, height - north);

             
                int south = (i == H - 1) ? 0 : A[i + 1][j];
                area += Math.Max(0, height - south);

           
                int west = (j == 0) ? 0 : A[i][j - 1];
                area += Math.Max(0, height - west);

             
                int east = (j == W - 1) ? 0 : A[i][j + 1];
                area += Math.Max(0, height - east);
            }
        }
